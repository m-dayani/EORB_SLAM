...
public:
    ...
    int SearchByProjection(Frame &F, const std::vector<MapPoint*> &vpMapPoints, float th=3,
            bool bFarPoints = false, float thFarPoints = 50.0f);
    ...
    int SearchForTriangulation(KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat F12,
            vector<pair<size_t, size_t> > &vMatchedPairs, bool bOnlyStereo, vector<cv::Mat> &vMatchedPoints);
    ...
    int Fuse(KeyFrame* pKF, const vector<MapPoint *> &vpMapPoints, float th=3.0, bool bRight = false);
...
protected:
    ...
    bool CheckDistEpipolarLine(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &F12, const KeyFrame *pKF, bool b1=false);
    bool CheckDistEpipolarLine2(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &F12, const KeyFrame *pKF, float unc);
