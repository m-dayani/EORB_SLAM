...
public:
    KeyFrame();
    explicit KeyFrame(const KeyFrame& KF);
    KeyFrame(Frame& F, Map* pMap, KeyFrameDatabase* pKFDB);

    KeyFrame(EORB_SLAM::EvFrame& F, int kfId, Map* pMap, KeyFrameDatabase* pKFDB);
    ...
    int numAllKPtsLeft() const { return this->NLeft; }
    int numAllKPtsRight() const { return this->NRight; }

    virtual std::vector<cv::KeyPoint> getAllUndistKPtsMono() const { return this->mvKeysUn; }
    virtual cv::KeyPoint getUndistKPtMono(const int idx) const;
    //virtual std::vector<cv::KeyPoint> getAllDistKPtsMono() const { return this->mvKeys; }
    virtual cv::KeyPoint getDistKPtMono(const int idx) const;
    //virtual std::vector<cv::KeyPoint> getAllKPtsRight() const { return this->mvKeysRight; }
    virtual cv::KeyPoint getKPtRight(const int idx) const;

    cv::Mat getAllORBDescriptors() const { return this->mDescriptors; }
    virtual cv::Mat getORBDescriptor(const int idx) const;
    virtual cv::Mat getDescriptorMono(const int idx);
    ...
    float getORBLevelSigma2(const int level) const {
        //assert(level >= 0 && level < mnScaleLevels);
    ...
    virtual int getKPtLevelMono(const int idx) const;
    virtual float getKPtScaleFactor(const int idx) const;
    virtual float getKPtLevelSigma2(const int idx) const;
    virtual float getKPtInvLevelSigma2(const int idx) const;
...
protected:
    ...
    virtual int numAllKPtsVir() const { return this->N; }
...
public:
    ...
    std::string printPointDistribution();

    KeyFrame* mpSynchOrbKF; // Used in Synch tracker to chain ORB-Ev KeyFrames
    KeyFrame* mpSynchEvKF; // Used in Synch trackerU (the other way of above)
    KeyFrame* mpOrigKF; // Original clone KF used in EvAsynchTracker
};
