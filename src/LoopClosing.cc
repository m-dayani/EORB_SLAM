    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            if(NewDetectCommonRegions())
            {
                if(mbMergeDetected)
                {
                    if ((mpTracker->mpSensor->isInertial()) &&
                    ...
                    else
                    {
                        ...
                        if(mpCurrentKF->GetMap()->IsInertial() && mpMergeMatchedKF->GetMap()->IsInertial())
                        {
                            ...
                            {
                                Eigen::Vector3d phi = Converter::LogSO3(mSold_new.rotation().toRotationMatrix());
                                phi(0)=0;
                                phi(1)=0;
                                mSold_new = g2o::Sim3(Converter::ExpSO3(phi),mSold_new.translation(),1.0);
                            }
                        }
...
//                        cout << "scale w2w1: " << mSold_new.scale() << endl;
                ...
                if(mbLoopDetected)
                {
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        Eigen::Vector3d phi = Converter::LogSO3(g2oSww_new.rotation().toRotationMatrix());
                        ...
                        if (fabs(phi(0))<0.008f && fabs(phi(1))<0.008f && fabs(phi(2))<0.349f)
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
                                    ...
                                    mpCurrentKF->GetMap()->GetIniertialBA2()) // TODO, maybe with GetIniertialBA1
                                {
                                    phi(0)=0;
                                    phi(1)=0;
                                    g2oSww_new = g2o::Sim3(Converter::ExpSO3(phi),g2oSww_new.translation(),1.0);
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    if(mpTracker->mpSensor->getConfig() == EORB_SLAM::MySensorConfig::STEREO && mpLastMap->GetAllKeyFrames().size() < 5) //12
    {
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
                                                 ...
                                                 std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    /*if(mpTracker->mpSensor==System::IMU_MONOCULAR ||mpTracker->mpSensor==System::IMU_STEREO)
    {
        nProjMatches = 50;
        nProjOptMatches = 50;
        nProjMatchesRep = 80;
    ...
    if(nNumProjMatches >= nProjMatches)
    {
        ...
        if(mpTracker->mpSensor->getConfig() == EORB_SLAM::MySensorConfig::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetIniertialBA2())
            bFixedScale=false;
        ...
        if(numOptMatches > nProjOptMatches)
        {
            ...
            vpMatchedMP.resize(mpCurrentKF->numAllKPts(), static_cast<MapPoint*>(NULL));
                                             ...
                                             int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    EORB_SLAM::MixedMatcher mixedMatcherBoW(0.9, true);
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        std::vector<MapPoint*> vpMatchedPoints = std::vector<MapPoint*>(mpCurrentKF->numAllKPts(), static_cast<MapPoint*>(NULL));
        std::vector<KeyFrame*> vpKeyFrameMatchedMP = std::vector<KeyFrame*>(mpCurrentKF->numAllKPts(), static_cast<KeyFrame*>(NULL));
        ...
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            ...
            int num;
            if (mpTracker->mFtDtMode == 0) {
                num = matcherBoW.SearchByBoW(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j]);
                //cout << "BoW: " << num << " putative matches with KF " << vpCovKFi[j]->mnId << endl;
            }
            else {
                num = mixedMatcherBoW.SearchByBoW(static_cast<EORB_SLAM::MixedKeyFrame*>(mpCurrentKF),
                        static_cast<EORB_SLAM::MixedKeyFrame*>(vpCovKFi[j]), vvpMatchedMPs[j]);
            }
        ...
        if(!bAbortByNearKF && numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            if(sConf == EORB_SLAM::MySensorConfig::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetIniertialBA2())
                bFixedScale=false;
            ...
            if(bConverge)
            {
                ...
                vpMatchedKF.resize(mpCurrentKF->numAllKPts(), static_cast<KeyFrame*>(NULL));

                int numProjMatches;
                if (mpTracker->mFtDtMode == 0) {
                    numProjMatches = matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpKeyFrames,
                                                                vpMatchedMP, vpMatchedKF, 8, 1.5);
                    cout << "BoW: " << numProjMatches << " matches between " << vpMapPoints.size()
                         << " points with coarse Sim3" << endl;
                }
                else {
                    auto* pCurrentMixedKF = static_cast<EORB_SLAM::MixedKeyFrame*>(mpCurrentKF);
                    numProjMatches = mixedMatcher.SearchByProjection(pCurrentMixedKF, mScw, vpMapPoints, vpKeyFrames,
                                                                vpMatchedMP, vpMatchedKF, 8, 1.5);
                    cout << "BoW: " << numProjMatches << " mixed matches between " << vpMapPoints.size()
                         << " points with coarse Sim3" << endl;
                }
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if (mpTracker->mFtDtMode == 0) {
                            numProjOptMatches = matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpMatchedMP,
                                                                           5, 1.0);
                            //cout <<"BoW: " << numProjOptMatches << " matches after of the Sim3 optimization" << endl;
                        }
                        else { // Mixed Features
                            auto* pCurrentMixedKF = static_cast<EORB_SLAM::MixedKeyFrame*>(mpCurrentKF);
                            numProjOptMatches = mixedMatcher.SearchByProjection(pCurrentMixedKF,
                                    mScw, vpMapPoints, vpMatchedMP, 5, 1.0);
                        }
                        ...
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            for(MapPoint* pMPi : vpMatchedMP)
                            {
                                ...
                                if(index >= 0)
                                {
                                    int coord_x = pKFi->getUndistKPtMono(index).pt.x;
                                    if(coord_x < min_x)
                                    {
                                        min_x = coord_x;
                                    }
                                    ...
                                    int coord_y = pKFi->getUndistKPtMono(index).pt.y;
                                    if(coord_y < min_y)
                                    {
                                        min_y = coord_y;
                                    }
                                         ...
                                         vector<MapPoint*> &vpMatchedMapPoints)
{
    ...
    vpMatchedMapPoints.resize(pCurrentKF->numAllKPts(), static_cast<MapPoint*>(NULL));
    int num_matches;
    if (mpTracker->mFtDtMode == 0) {
        num_matches = matcher.SearchByProjection(pCurrentKF, mScw, vpMapPoints, vpMatchedMapPoints, 3, 1.5);
    }
    else {
        auto* pCurrentMixedKF = static_cast<EORB_SLAM::MixedKeyFrame*>(pCurrentKF);
        num_matches = mixedMatcher.SearchByProjection(pCurrentMixedKF, mScw, vpMapPoints,
...
void LoopClosing::CorrectLoop()
{
    ...
    if(mpTracker->mpSensor->getConfig() == EORB_SLAM::MySensorConfig::IMU_MONOCULAR && !mpCurrentKF->GetMap()->GetIniertialBA2())
        bFixedScale=false;
...
void LoopClosing::MergeLocal()
{
    ...
    else
    {
        ...
        {
            if(sConf == EORB_SLAM::MySensorConfig::MONOCULAR)
            {
                unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate); // We update the current map with the Merge information
...
void LoopClosing::printReprojectionError(set<KeyFrame*> &spLocalWindowKFs, KeyFrame* mpCurrentKF, string &name)
{
    ...
    for(KeyFrame* pKFi : spLocalWindowKFs)
    {
        ...
        for(int j=0; j<vpMPs.size(); ++j)
        {
            ...
            cv::KeyPoint point_img = pKFi->getUndistKPtMono(j);
            cv::Point2f reproj_p;
            float u, v;
...
void LoopClosing::MergeLocal2()
{
    ...
    const bool bIsEvent = mpTracker->mpSensor->isEvent();

    {
        ...
        IMU::Bias b_on = mpCurrentKF->GetImuBias();
        KeyFrame* pLastKFon = mpTracker->GetLastKeyFrame();
        mpTracker->UpdateFrameIMU(s_on,b_on,pLastKFon);

        std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();

        if (bIsEvent) {
            mpTracker->mpSystem->ApplyScaleAndRotationEvSynch(R_on,s_on,bScaleVel,t_on);
            mpTracker->mpSystem->UpdateEvFrameImuSynch(s_on,b_on,pLastKFon);
        }
    }

    const int numKFnew=pCurrentMap->KeyFramesInMap();

    if((mpTracker->mpSensor->isInertial()) && !pCurrentMap->GetIniertialBA2()){
        ...
        unique_lock<mutex> lock(mpAtlas->GetCurrentMap()->mMutexMapUpdate);
        KeyFrame* pLastKFm2 = mpTracker->GetLastKeyFrame();
        mpTracker->UpdateFrameIMU(1.0f,b,pLastKFm2);

        if (bIsEvent) {
            mpTracker->mpSystem->UpdateEvFrameImuSynch(1.0f,b,pLastKFm2);
        }
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap, vector<MapPoint*> &vpMapPoints)
{
    ...
    EORB_SLAM::MixedMatcher mixedMatcher(0.8);
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        int numFused;
        if (mpTracker->mFtDtMode == 0) {
            numFused = matcher.Fuse(pKFi, cvScw, vpMapPoints, 4, vpReplacePoints);
        }
        else {
            numFused = mixedMatcher.Fuse(static_cast<EORB_SLAM::MixedKeyFrame*>(pKFi), cvScw,
                                         vpMapPoints, 4, vpReplacePoints);
        }
...
void LoopClosing::SearchAndFuse(const vector<KeyFrame*> &vConectedKFs, vector<MapPoint*> &vpMapPoints)
{
    ...
    for(auto mit=vConectedKFs.begin(), mend=vConectedKFs.end(); mit!=mend;mit++)
    {
        ...
        if (mpTracker->mFtDtMode == 0) {
            ...
            mixedMatcher.Fuse(static_cast<EORB_SLAM::MixedKeyFrame*>(pKF), cvScw, vpMapPoints, 4, vpReplacePoints);
        }
...
void LoopClosing::RunGlobalBundleAdjustment(Map* pActiveMap, unsigned long nLoopKF)
{
    ...
    if(!bImuInit) {
//        if (mpTracker->mpSensor->isEvent()) {
//            // If we have events, do a global event-image bundle adjustment
//            vector<KeyFrame*> vpEvKFs;
//            mpTracker->mpSystem->GetEventConstraints(vpEvKFs);
//
//            if (vpEvKFs.empty()) {
//                Optimizer::GlobalBundleAdjustemnt(pActiveMap, 10, &mbStopGBA, nLoopKF, false);
//            }
//            else {
//                EORB_SLAM::MyOptimizer::GlobalBundleAdjustment(pActiveMap, vpEvKFs, 10, &mbStopGBA, nLoopKF, false);
//                // TODO: Also signal Event Tracker to perform a pose only BA
//            }
//        }
//        else {

        if (bIsEvent) {
            EORB_SLAM::EvOptimizer::GlobalBundleAdjustemnt(pActiveMap, 10, &mbStopGBA, nLoopKF, false);
        }
        else {
            Optimizer::GlobalBundleAdjustemnt(pActiveMap, 10, &mbStopGBA, nLoopKF, false);
        }
//        }
        ...
        if (bIsEvent) {
            EORB_SLAM::EvOptimizer::FullInertialBA(pActiveMap, 7, false, nLoopKF, &mbStopGBA);
        }
        else {
