...
MapDrawer::MapDrawer(Atlas* pAtlas, const EORB_SLAM::ViewerParamsPtr pViewerParams):
    mpAtlas(pAtlas), mpViewerParams(pViewerParams)
{
    //cv::FileStorage fSettings(strSettingPath, cv::FileStorage::READ);

    //bool is_correct = ParseViewerParamFile(fSettings);

    if(!mpViewerParams || mpViewerParams->missParams)
    {
        std::cerr << "**MapDrawer::MapDrawer: ERROR in the config file, the format is not correct**" << std::endl;
        try
        {
            throw -1;
        }
...
void MapDrawer::DrawMapPoints()
{
    ...
    glPointSize(mpViewerParams->mPointSize);
    glBegin(GL_POINTS);
...
{
    const float &w = mpViewerParams->mKeyFrameSize;
    ...
    if(bDrawKF)
    {
        for(size_t i=0; i<vpKFs.size(); i++)
        {
            ...
            try {
                Twc = pKF->GetPoseInverse().t();
            }
            catch (const cv::Exception& e) {
                LOG(ERROR) << e.what() << endl;
            }
            catch (...) {
                LOG(ERROR) << "MapDrawer::DrawKeyFrames: Unknown exception thrown (retrieving KF's Pose Inverse\n";
            }
            ...
            {
                glLineWidth(mpViewerParams->mKeyFrameLineWidth*5);
            ...
            {
                glLineWidth(mpViewerParams->mKeyFrameLineWidth);
    ...
    if(bDrawGraph)
    {
        glLineWidth(mpViewerParams->mGraphLineWidth);
...
{
    const float &w = mpViewerParams->mCameraSize;
    const float h = w*0.75;
    const float z = w*0.6;
    ...
    glLineWidth(mpViewerParams->mCameraLineWidth);
