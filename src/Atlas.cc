...
Atlas::Atlas(int initKFid, int initMapId): mnLastInitKFidMap(initKFid), mHasViewer(false)
{
    mpCurrentMap = static_cast<Map*>(NULL);
    CreateNewMap(initMapId);
}
...
void Atlas::CreateNewMap()
{
    ...
    DLOG(INFO) << "Creation of new map with id: " << Map::nNextId << endl;

    if(mpCurrentMap){
        LOG(INFO) << "Exits current map " << endl;
        if(!mspMaps.empty() && mnLastInitKFidMap < mpCurrentMap->GetMaxKFid())
            mnLastInitKFidMap = mpCurrentMap->GetMaxKFid()+1; //The init KF is the next of current maximum

        mpCurrentMap->SetStoredMap();
        LOG(INFO) << "Saved map with ID: " << mpCurrentMap->GetId() << endl;

        //if(mHasViewer)
        //    mpViewer->AddMapToCreateThumbnail(mpCurrentMap);
    }
    DLOG(INFO) << "Creation of new map with last KF id: " << mnLastInitKFidMap << endl;
...
void Atlas::CreateNewMap(int initMapId)
{
    unique_lock<mutex> lock(mMutexAtlas);
    ...
    mpCurrentMap = new Map(mnLastInitKFidMap, initMapId);
    mpCurrentMap->SetCurrentMap();
    mspMaps.insert(mpCurrentMap);
}
...
{
    unique_lock<mutex> lock(mMutexAtlas);
    LOG(INFO) << "Chage to map with id: " << pMap->GetId() << endl;
    if(mpCurrentMap){
        mpCurrentMap->SetStoredMap();
    }
...
void Atlas::PreSave()
{
    ...
    LOG(INFO) << "There are " << spCams.size() << " cameras in the atlas" << endl;
    for(Map* pMi : mvpBackupMaps)
    {
        LOG(INFO) << "Pre-save of map " << pMi->GetId() << endl;
        pMi->PreSave(spCams);
    }
    LOG(INFO) << "Maps stored" << endl;
    for(GeometricCamera* pCam : mvpCameras)
    {
        LOG(INFO) << "Pre-save of camera " << pCam->GetId() << endl;
...
void Atlas::PostLoad()
{
    ...
    for(Map* pMi : mvpBackupMaps)
    {
        LOG(INFO) << "Map id:" << pMi->GetId() << endl;
        mspMaps.insert(pMi);
    ...
    LOG(INFO) << "Number KF:" << numKF << "; number MP:" << numMP << endl;
    mvpBackupMaps.clear();
}
