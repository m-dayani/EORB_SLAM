...
void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag,
...
{    
    ...
    if(num_fixedKF == 0)
    {
        LOG(WARNING) << "LM-LBA: There are 0 fixed KF in the optimizations, LBA aborted\n";
        //return;
    }
    //Verbose::PrintMess("LM-LBA: There are " + to_string(lLocalKeyFrames.size()) + " KFs and " + to_string(lLocalMapPoints.size()) + " MPs to optimize. " + to_string(num_fixedKF) + " KFs are fixed", Verbose::VERBOSITY_DEBUG);

    if (lLocalKeyFrames.empty() ||
        (lLocalKeyFrames.size() == 1 && (*(lLocalKeyFrames.begin()))->mnId == pMap->GetInitKFid())) {
    ...
    const int nAllKFs = lLocalKeyFrames.size()+lFixedCameras.size();
    ...
    // Set MapPoint vertices
    const int nExpectedSize = nAllKFs * static_cast<int>(lLocalMapPoints.size());
        ...
        const float th2, const bool bFixScale)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        const cv::KeyPoint &kpUn1 = pKF1->getUndistKPtMono(i);
        ...
        const float &invSigmaSquare1 = pKF1->getKPtInvLevelSigma2(i);
        ...
        const cv::KeyPoint &kpUn2 = pKF2->getUndistKPtMono(i2);
        ...
        float invSigmaSquare2 = pKF2->getKPtInvLevelSigma2(i2);
                            ...
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(i2 >= 0)
        {
            kpUn2 = pKF2->getUndistKPtMono(i2);
            obs2 << kpUn2.pt.x, kpUn2.pt.y;

            invSigmaSquare2 = pKF2->getKPtInvLevelSigma2(i2);
        ...
        else
        {
            ...
            invSigmaSquare2 = pKF2->getORBInvLevelSigma2(kpUn2.octave);
            if (!pMP2->isORBMapPoint()) {
                auto* pMixedKFm = dynamic_cast<EORB_SLAM::MixedKeyFrame*>(pKF2);
                if (pMixedKFm)
                    invSigmaSquare2 = pMixedKFm->getAKAZEInvLevelSigma2(kpUn2.octave);
            }
    ...
    Verbose::PrintMess("Sim3: There are " + to_string(nCorrespondences) + " matches, " + to_string(nInKF2) +
        " are in the KF and " + to_string(nOutKF2) + " are in the connected KFs. There are " + to_string(nMatchWithoutMP) +
                            ...
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(i2 >= 0 && pKFm == pKF2)
        {
            kpUn2 = pKFm->getUndistKPtMono(i2);
            obs2 << kpUn2.pt.x, kpUn2.pt.y;
            invSigmaSquare2 = pKFm->getKPtInvLevelSigma2(i2);
        }
        else
        {
            float invz = 1/P3D2c.at<float>(2);
            ...
            invSigmaSquare2 = pKFm->getORBInvLevelSigma2(kpUn2.octave);
            if (!pMP2->isORBMapPoint()) {
                auto* pMixedKFm = dynamic_cast<EORB_SLAM::MixedKeyFrame*>(pKFm);
                if (pMixedKFm)
                    invSigmaSquare2 = pMixedKFm->getAKAZEInvLevelSigma2(kpUn2.octave);
            }
        }
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                if(leftIndex != -1 && pKFi->mvuRight[leftIndex]<0)
                {
                    ...
                    const float &invSigma2 = pKFi->getKPtInvLevelSigma2(leftIndex)/unc2;
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
                ...
                else if(leftIndex != -1)// Stereo observation
                {
                    ...
                    const float &invSigma2 = pKFi->getORBInvLevelSigma2(kpUn.octave)/unc2;
                    e->setInformation(Eigen::Matrix3d::Identity()*invSigma2);
        ...
        Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA)
{
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pPreKF && pKFi->mnId<=maxKFid && pPreKF->GetMap() == pMap)
        {
            if(pKFi->isBad() || pPreKF->mnId>maxKFid)
                continue;
            ...
            pKFi->mpImuPreintegrated->SetNewBias(pPreKF->GetImuBias());
            g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pPreKF->mnId);
            g2o::HyperGraph::Vertex* VV1 = optimizer.vertex(maxKFid+(pPreKF->mnId)+1);
            g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(pKFi->mnId);
            ...
            vppUsedKF.push_back(make_pair(pPreKF,pKFi));
            optimizer.addEdge(ei);
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale)
{
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pPreKF && pKFi->mnId<=maxKFid && pPreKF->GetMap() == pMap)
        {
            ...
            g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pPreKF->mnId);
            g2o::HyperGraph::Vertex* VV1 = optimizer.vertex((maxKFid+1)+pPreKF->mnId);
            g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(pKFi->mnId);
            g2o::HyperGraph::Vertex* VV2 = optimizer.vertex((maxKFid+1)+pKFi->mnId);
            g2o::HyperGraph::Vertex* VG = optimizer.vertex(2*(maxKFid+1)+pPreKF->mnId);
            g2o::HyperGraph::Vertex* VA = optimizer.vertex(3*(maxKFid+1)+pPreKF->mnId);
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame* pCurrentKF, vector<KeyFrame*> vpWeldingKFs, vector<KeyFrame*> vpFixedKFs, bool *pbStopFlag)
{
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            const cv::KeyPoint &kpUn = pKF->getUndistKPtMono(get<0>(mit->second));
            ...
            if(pKF->mvuRight[get<0>(mit->second)]<0) //Monocular
            {
                ...
                e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKF->mnId)));
                e->setMeasurement(obs);
                const float &invSigma2 = pKF->getKPtInvLevelSigma2(get<0>(mit->second));
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF,vector<KeyFrame*> vpAdjustKF, vector<KeyFrame*> vpFixedKF, bool *pbStopFlag)
{
    ...
    if ((nFixedKFs+nNonFixedKFs) <= 0) {
        LOG(WARNING) << "Merge-LBA: No KF vertex can be added, abort optimization...\n";
        return;
    }
    if (nNonFixedKFs <= 0) {
        LOG(WARNING) << "Merge-LBA: No Non-Fixed KF vertex can be added but SBA not supported, abort optimization...\n";
        return;
    }
    ...
    if (nVertexMPs <= 0) {
        LOG(ERROR) << "Merge-LBA: Num Non-Fixed KFs: " << nNonFixedKFs
                   << ", Num fixed: " << nFixedKFs << ", but no MPs can be added, abort optimization...\n";
        return;
    }
    ...
    DLOG(INFO) << "Merge-LBA: nFixedKFs: " << nFixedKFs << ", nNonFixedKFs: " << nNonFixedKFs
               << ", nMPs: " << nVertexMPs << endl;

    // Make sure we have vertices to optimize!
    // Actually, the main reason for nasty exceptions are that there are some MPs
    // that are connected only to fixed KFs and since they are set to marginalize -> Boom!
    bool allVerticesMarginalized = true;
    ...
    if (allVerticesMarginalized) {
        LOG(ERROR) << "Merge-LBA: All vertices set to marginalize, abort...\n";
        return;
    }
    ...
    if(bDoMore)
    {
        ...
        Verbose::PrintMess("LBA: First optimization, there are " + to_string(badMonoMP) +
                    " monocular and " + to_string(badStereoMP) + " sterero bad edges", Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess("LBA: Second optimization, there are " + to_string(badMonoMP) +
                    " monocular and " + to_string(badStereoMP) + " sterero bad edges", Verbose::VERBOSITY_DEBUG);
        ...
        if(dist > 1.0)
        {
            if(bShowImages)
            {
                ...
                for(int i=0; i<vpMonoMPsOpt.size(); ++i)
                {
                    ...
                    cv::circle(imLeft, pKFi->getDistKPtMono(index).pt, 2, cv::Scalar(255, 0, 0));
                 ...
                 for(int i=0; i<vpStereoMPsOpt.size(); ++i)
                 {
                     ...
                     if(index < 0)
                     {
                      ...
                      //string strNumOBs = to_string(vpMapPointsKF[i]->Observations());
                      cv::circle(imLeft, pKFi->getDistKPtMono(index).pt, 2, cv::Scalar(0, 255, 0));
                      //cv::putText(imLeft, strNumOBs, pKF->mvKeys[i].pt, CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0, 0));
                      numPointsStereo++;
                 }
                 ...
                 for(int i=0; i<vpMonoMPsBad.size(); ++i)
                 {
                    ...
                    //string strNumOBs = to_string(vpMapPointsKF[i]->Observations());
                    cv::circle(imLeft, pKFi->getDistKPtMono(index).pt, 2, cv::Scalar(0, 0, 255));
                    //cv::putText(imLeft, strNumOBs, pKF->mvKeys[i].pt, CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0, 0));
                    numPointsMonoBad++;
                }
    ...
    {
        MapPoint* pMPi = vpMPs[i];
        if(pMPi->isBad() || vbNotIncludedMP[i])
            continue;
        ...
        LoopClosing::KeyFrameAndPose &corrPoses)
{
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                const cv::KeyPoint &kpUn = pKFi->getUndistKPtMono(get<0>(mit->second));
                ...
                if(pKFi->mvuRight[get<0>(mit->second)]<0) // Monocular observation
                {
                    ...
                    e->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(pKFi->mnId)));
                    e->setMeasurement(obs);
                    const float &invSigma2 = pKFi->getKPtInvLevelSigma2(get<0>(mit->second));
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2);
