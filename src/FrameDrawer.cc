...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    //mvCurrentKeys=pTracker->mpCurrentFrame->getAllDistKPtsMono();
    int nAllDistKPtsMono = pTracker->mpCurrentFrame->numAllKPts();
    mvCurrentKeys.resize(nAllDistKPtsMono);

    if(both){
        mvCurrentKeysRight = pTracker->mpCurrentFrame->getAllKPtsRight();
        pTracker->mImRight.copyTo(mImRight);
        N = nAllDistKPtsMono + mvCurrentKeysRight.size();
    }
    else{
        N = nAllDistKPtsMono;
    }
    ...
    // TODO: Is this right for mixed features????
    mCurrentFrame = *(pTracker->mpCurrentFrame);
    mmProjectPoints = mCurrentFrame.mmProjectPoints;
    //mmMatchedInImage = mpCurrentFrame.mmMatchedInImage;
    ...
    {
        mvIniKeys=pTracker->mpInitialFrame->getAllDistKPtsMono();
    ...
    else if(pTracker->mLastProcessedState==Tracking::OK)
    {
        for(int i=0;i<N;i++)
        {
            MapPoint* pMP = pTracker->mpCurrentFrame->getMapPoint(i);
            if(pMP)
            {
                mvCurrentKeys[i] = pTracker->mpCurrentFrame->getDistKPtMono(i);
                if(!pTracker->mpCurrentFrame->getMPOutlier(i))
                {
                    ...
                    //cv::Point2f point3d_proy;
                    //float u, v;
                    //bool bIsInImage = mpCurrentFrame.ProjectPointDistort(pMP, point3d_proy, u, v);
                    //if(bIsInImage)
                    //{
                        //mvMatchedKeys.push_back(mvCurrentKeys[i]);
