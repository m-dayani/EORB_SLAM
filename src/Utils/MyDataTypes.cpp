...
namespace EORB_SLAM {
    ...
    void FrameInfo::setIsLostRange(ulong iniKFrId, ulong lastKFrId, const bool isLost, const bool resetAll) {

        // check data integrity
        assert(this->checkDataIntegrity());

        // set all as lost except range
        auto kfIter = mlpReferences.begin();
        uint frcnt = 0;
        for (auto iter = mlbLost.begin(); iter != mlbLost.end(); iter++, kfIter++) {

            if (resetAll) {
                *iter = ((*kfIter)->mnId >= iniKFrId && (*kfIter)->mnId <= lastKFrId) == isLost;
            }
            else if ((*kfIter)->mnId >= iniKFrId && (*kfIter)->mnId <= lastKFrId) {
                *iter = true;
            }
        ...
        LOG(INFO) << "FrameInfo::setIsLostRange: " << frcnt << " frames changed\n";
    }

    void FrameInfo::setIsLostRange(double iniFrTs, double lastFrTs, const bool isLost, const bool resetAll) {

        // check data integrity
        assert(this->checkDataIntegrity());

        // set all as lost except range
        auto kfIter = mlFrameTimes.begin();
        uint frcnt = 0;
        for (auto iter = mlbLost.begin(); iter != mlbLost.end(); iter++, kfIter++) {

            if (resetAll) {
                *iter = (*kfIter >= iniFrTs && *kfIter <= lastFrTs) == isLost;
            }
            else if (*kfIter >= iniFrTs && *kfIter <= lastFrTs) {
                *iter = true;
            }
