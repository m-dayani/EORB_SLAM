    ...
    void MyFrameDrawer::draw(cv::Mat &toShow) {
        ...
        if (mnChannels > 2) {
            ...
            for (int i = 0; i < mnChannels; i++) {
                ...
                vImagesToShow[i].copyTo(toShow.rowRange(i*maxRows, (i+1)*maxRows).colRange(i*maxCols, (i+1)*maxCols));
            }
        }
        else {
            if (mnChannels == 1) {
                ...
                vImagesToShow[0].copyTo(toShow.rowRange(0, maxRows).colRange(0, maxCols));
                vImagesToShow[1].copyTo(toShow.rowRange(0, maxRows).colRange(maxCols, 2*maxCols));
            }
            else {
