    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        if(pMP->mbTrackInView)
        {
            ...
            const vector<size_t> vIndices =
                    F.GetFeaturesInArea(pMP->mTrackProjX,pMP->mTrackProjY,r*F.getORBScaleFactor(nPredictedLevel),
            ...
            if(!vIndices.empty()){
                ...
                for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
                {
                    ...
                    if(F.numKPtsLeft() == -1 && F.mvuRight[idx]>0)
                    {
                        ...
                        if(er>r*F.getORBScaleFactor(nPredictedLevel))
                            continue;
                    }

                    const cv::Mat &d = F.getORBDescriptor(idx);
        ...
        if(F.numKPtsLeft() != -1 && pMP->mbTrackInViewR){
            ...
            if(nPredictedLevel != -1){
                ...
                const vector<size_t> vIndices =
                        F.GetFeaturesInArea(pMP->mTrackProjXR,pMP->mTrackProjYR,r*F.getORBScaleFactor(nPredictedLevel),
...
bool ORBmatcher::CheckDistEpipolarLine(const cv::KeyPoint &kp1,const cv::KeyPoint &kp2,
        const cv::Mat &F12,const KeyFrame* pKF2, const bool b1)
{
    ...
    if(!b1)
        return dsqr<3.84*pKF2->getORBLevelSigma2(kp2.octave);
    else
        return dsqr<6.63*pKF2->getORBLevelSigma2(kp2.octave);
}

bool ORBmatcher::CheckDistEpipolarLine2(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2,
        const cv::Mat &F12, const KeyFrame *pKF2, const float unc)
{
    // Epipolar line in second image l = x1'F12 = [a b c]
    const float a = kp1.pt.x*F12.at<float>(0,0)+kp1.pt.y*F12.at<float>(1,0)+F12.at<float>(2,0);
    const float b = kp1.pt.x*F12.at<float>(0,1)+kp1.pt.y*F12.at<float>(1,1)+F12.at<float>(2,1);
    ...
    if(unc==1.f)
        ...
        return dsqr<3.84*pKF2->getORBLevelSigma2(kp2.octave)*unc;
}
    ...
    while(KFit != KFend && Fit != Fend)
    {
        if(KFit->first == Fit->first)
        {
            ...
            for(size_t iKF=0; iKF<vIndicesKF.size(); iKF++)
            {
                ...
                const cv::Mat &dKF= pKF->getORBDescriptor(realIdxKF);
                ...
                for(size_t iF=0; iF<vIndicesF.size(); iF++)
                {
                    if(F.numKPtsLeft() == -1){
                        ...
                        const cv::Mat &dF = F.getORBDescriptor(static_cast<int>(realIdxF));
                                   ...
                                   vector<MapPoint*> &vpMatched, int th, float ratioHamming)
{
    ...
    for(int iMP=0, iendMP=vpPoints.size(); iMP<iendMP; iMP++)
    {
        ...
        // Search in a radius
        const float radius = th*pKF->getORBScaleFactor(nPredictedLevel);
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::Mat &dKF = pKF->getORBDescriptor(idx);
...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const std::vector<MapPoint*> &vpPoints,
        ...
        vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=F1.numAllKPts(); i1<iend1; i1++)
    {
        ...
        cv::Mat d1 = F1.getORBDescriptor(i1);
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            cv::Mat d2 = F2.getORBDescriptor(i2);
    ...
    while(f1it != f1end && f2it != f2end)
    {
        if(f1it->first == f2it->first)
        {
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                if(pKF1 -> numAllKPtsLeft() != -1 && idx1 >= pKF1 -> numAllKPts()){
                    continue;
                }
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    if(pKF2 -> numAllKPtsLeft() != -1 && idx2 >= pKF2 -> numAllKPts()){
                        continue;
                    }
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
        if(f1it->first == f2it->first)
        {
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                //if(bRight1) continue;
                const cv::Mat &d1 = pKF1->getORBDescriptor(idx1);
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    const cv::Mat &d2 = pKF2->getORBDescriptor(idx2);
        ...
        vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo, vector<cv::Mat> &vMatchedPoints)
{
    ...
    vector<cv::Mat> vMatchesPoints12(pKF1->numAllKPts());
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
        if(f1it->first == f2it->first)
        {
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                ...
                const bool bRight1 = (pKF1 -> numAllKPtsLeft() == -1 || idx1 < pKF1 -> numAllKPtsLeft()) ? false
                                                                                   : true;
                ...
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    ...
                    const cv::KeyPoint &kp2 = (pKF2 -> numAllKPtsLeft() == -1) ? pKF2->getUndistKPtMono(idx2)
                                                                    : (idx2 < pKF2 -> numAllKPtsLeft()) ? pKF2 -> getDistKPtMono(idx2)
                                                                                             : pKF2 -> getKPtRight(idx2 - pKF2 -> numAllKPtsLeft());
                    const bool bRight2 = (pKF2 -> numAllKPtsLeft() == -1 || idx2 < pKF2 -> numAllKPtsLeft()) ? false
                                                                                       : true;
                    ...
                    cv::Mat x3D;
                    if(pCamera1->matchAndtriangulate(kp1,kp2,pCamera2,Tcw1,Tcw2,pKF1->getORBLevelSigma2(kp1.octave),pKF2->getORBLevelSigma2(kp2.octave),x3D)){
                        bestIdx2 = idx2;
                        bestDist = dist;
                        bestPoint = x3D;
                    }
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            if(pKF->mvuRight[idx]>=0)
            {
                ...
                if(e2*pKF->getORBInvLevelSigma2(kpLevel)>7.8)
                    continue;
            }
            else
            {
                ...
                if(e2*pKF->getORBInvLevelSigma2(kpLevel)>5.99)
                    continue;
            }
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        // Search in a radius
        const float radius = th*pKF2->getORBScaleFactor(nPredictedLevel);
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::KeyPoint &kp = pKF2->getUndistKPtMono(idx);

            if(kp.octave<nPredictedLevel-1 || kp.octave>nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF2->getORBDescriptor(idx);
    ...
    for(int i2=0; i2<N2; i2++)
    {
        ...
        const float radius = th*pKF1->getORBScaleFactor(nPredictedLevel);
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            const cv::KeyPoint &kp = pKF1->getUndistKPtMono(idx);

            if(kp.octave<nPredictedLevel-1 || kp.octave>nPredictedLevel)
                continue;

            const cv::Mat &dKF = pKF1->getORBDescriptor(idx);
    ...
    for(int i=0; i<LastFrame.numAllKPts(); i++)
    {
        ...
        if(pMP)
        {
            if(!LastFrame.getMPOutlier(i))
            {
                ...
                float radius = th*CurrentFrame.getORBScaleFactor(nLastOctave);
                ...
                for(vector<size_t>::const_iterator vit=vIndices2.begin(), vend=vIndices2.end(); vit!=vend; vit++)
                {
                    ...
                    const cv::Mat &d = CurrentFrame.getORBDescriptor(i2);
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad() && !sAlreadyFound.count(pMP))
            {
                ...
                // Search in a window
                const float radius = th*CurrentFrame.getORBScaleFactor(nPredictedLevel);
...
{
    assert(a.rows == b.rows && a.cols == b.cols && !a.empty() && !b.empty());
