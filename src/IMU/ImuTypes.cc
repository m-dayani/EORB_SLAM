...
void Preintegrated::IntegrateNewMeasurement(const cv::Point3f &acceleration, const cv::Point3f &angVel, const float &dt)
{
    ...
    dR = Converter::NormalizeRotation(dR*dRi.deltaR);
...
cv::Mat Preintegrated::GetDeltaRotation(const Bias &b_)
{
    ...
    return Converter::NormalizeRotation(dR*Converter::ExpSO3(JRg*dbg));
}
...
{
    std::unique_lock<std::mutex> lock(mMutex);
    return Converter::NormalizeRotation(dR*Converter::ExpSO3(JRg*db.rowRange(0,3)));
}
