            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        if (bIsEvent) {
                            EORB_SLAM::EvOptimizer::LocalInertialBA(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(),
                    ...
                    else
                    {
                        ...
                        if (bIsEvent) {
                            ...
                            EORB_SLAM::EvOptimizer::LocalBundleAdjustment(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(),
                                    num_FixedKF_BA, num_OptKF_BA, num_MPs_BA, num_edges_BA);
                        }
                        else {
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    EORB_SLAM::MixedMatcher mixedMatcher(th, false);
    auto* pCurrentMixedKF = dynamic_cast<EORB_SLAM::MixedKeyFrame*>(mpCurrentKeyFrame);
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        if (mpTracker->mFtDtMode == 0) {
            ...
            mixedMatcher.SearchForTriangulation(pCurrentMixedKF, pMixedKF2, F12, vMatchedIndices, false, bCoarse);
        }
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            bool isORBKPt1 = true;
            if (pCurrentMixedKF) {
                isORBKPt1 = pCurrentMixedKF->isORBDescValid(idx1);
            }
            bool isORBKPt2 = true;
            if (pMixedKF2) {
            ...
            if (!isORBKPt1 && !isORBKPt2)
                ratioFactor = 1.5f * pCurrentMixedKF->getAKAZEScaleFactor();
...
void LocalMapping::SearchInNeighbors()
{
    ...
    ORBmatcher matcher;
    EORB_SLAM::MixedMatcher mixedMatcher;
    ...
    for(vector<KeyFrame*>::iterator vit=vpTargetKFs.begin(), vend=vpTargetKFs.end(); vit!=vend; vit++)
    {
        ...
        if (mpTracker->mFtDtMode == 0) {
            ...
            if (pKFi->numAllKPtsLeft() != -1) matcher.Fuse(pKFi, vpMapPointMatches, true);
        }
        else {
            auto* pMixedKFi = static_cast<EORB_SLAM::MixedKeyFrame*>(pKFi);
            mixedMatcher.Fuse(pMixedKFi, vpMapPointMatches);
            if (pKFi->numAllKPtsLeft() != -1) mixedMatcher.Fuse(pMixedKFi, vpMapPointMatches, true);
        }
    }
    ...
    if (mpTracker->mFtDtMode == 0) {
        ...
        if (mpCurrentKeyFrame->numAllKPtsLeft() != -1) matcher.Fuse(mpCurrentKeyFrame, vpFuseCandidates, true);
    }
    else {
        auto* pCurrentMixedKF = static_cast<EORB_SLAM::MixedKeyFrame*>(mpCurrentKeyFrame);
        mixedMatcher.Fuse(pCurrentMixedKF, vpFuseCandidates);
        if (pCurrentMixedKF->numAllKPtsLeft() != -1) mixedMatcher.Fuse(pCurrentMixedKF, vpFuseCandidates, true);
    }
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    if (!mpCurrentKeyFrame->GetMap()->isImuInitialized())
    {
        ...
        for(vector<KeyFrame*>::iterator itKF = vpKF.begin(); itKF!=vpKF.end(); itKF++)
        {
            ...
            // TODO: How??? dirG -= Rwb(j-1) * dVb(j-1)(j)
    ...
    mInitTime = mpTracker->mpLastFrame->mTimeStamp - vpKF.front()->mTimeStamp;
    ...
    if ((fabs(mScale-1.f)>0.00001)||!mbMonocular)
    {
        ...
        IMU::Bias b = vpKF[0]->GetImuBias();
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(R,mScale,true);
        mpTracker->UpdateFrameIMU(mScale,b,mpCurrentKeyFrame);

        if (bIsEvent) {
            mpSystem->ApplyScaleAndRotationEvSynch(R,mScale,true,cv::Mat::zeros(3,1,CV_32FC1));
            mpSystem->UpdateEvFrameImuSynch(mScale,b,mpCurrentKeyFrame);
        }
    }
    std::chrono::steady_clock::time_point t3 = std::chrono::steady_clock::now();
    ...
    if (bFIBA)
    {
        ...
        if (mpTracker->mpSensor->isEvent()) {
            EORB_SLAM::EvOptimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0,
                                                   NULL, bInit, fpriorG, fpriorA);
        }
        else {
            Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0,
                                      NULL, bInit, fpriorG, fpriorA);
        }
    }
    ...
    if (bIsEvent) {
        mpSystem->UpdateEvFrameImuSynch(1.0,vpKF[0]->GetImuBias(),mpCurrentKeyFrame);
    }
    if (!mpAtlas->isImuInitialized())
    {
...
void LocalMapping::ScaleRefinement()
{
    ...
    {
        cv::Mat R = Converter::toCvMat(mRwg).t();
        IMU::Bias b = mpCurrentKeyFrame->GetImuBias();
        mpAtlas->GetCurrentMap()->ApplyScaledRotation(R,mScale,true);
        mpTracker->UpdateFrameIMU(mScale,b,mpCurrentKeyFrame);
