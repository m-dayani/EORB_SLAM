...
Frame::Frame(): mpcpi(NULL), mpImuPreintegrated(NULL), mpPrevFrame(NULL), mpImuPreintegratedFrame(NULL),
    mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbImuPreintegrated(false), mbf(0.f), mThDepth(0.f), mnMatches(0),
    mpEvFrame(nullptr)
{
}
...
Frame::Frame(const Frame &frame)
    : mpcpi(frame.mpcpi), mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft),
      ...
      mbf(frame.mbf), mb(frame.mb), mThDepth(frame.mThDepth), N(frame.numAllKPts()), mvKeys(frame.getAllORBDistKPtsMono()),
      mvKeysRight(frame.getAllKPtsRight()), mvKeysUn(frame.getAllORBUndistKPtsMono()), mvuRight(frame.mvuRight),
      mvDepth(frame.mvDepth), mBowVec(frame.mBowVec), mFeatVec(frame.mFeatVec),
      mDescriptors(frame.getAllORBDescMono().clone()), mDescriptorsRight(frame.mDescriptorsRight.clone()),
      mvpMapPoints(frame.getAllMapPointsMono()), mvbOutlier(frame.getAllOutliersMono()), mImuCalib(frame.mImuCalib),
      mnCloseMPs(frame.mnCloseMPs), mpImuPreintegrated(frame.mpImuPreintegrated),
      mpImuPreintegratedFrame(frame.mpImuPreintegratedFrame), mImuBias(frame.mImuBias),
      mnId(frame.mnId), mpReferenceKF(frame.mpReferenceKF), mnScaleLevels(frame.getORBNLevels()),
      mfScaleFactor(frame.getORBScaleFactor()), mfLogScaleFactor(frame.getORBLogScaleFactor()),
      mvScaleFactors(frame.getAllORBScaleFactors()), mvInvScaleFactors(frame.getAllORBInvScaleFactors()),
      mNameFile(frame.mNameFile), mnDataset(frame.mnDataset),
      mvLevelSigma2(frame.getAllORBLevelSigma2()), mvInvLevelSigma2(frame.getAllORBInvLevelSigma2()), mpPrevFrame(frame.mpPrevFrame),
      mpLastKeyFrame(frame.mpLastKeyFrame), mbImuPreintegrated(frame.mbImuPreintegrated), mpMutexImu(frame.mpMutexImu),
      mpCamera(frame.mpCamera), mpCamera2(frame.mpCamera2), Nleft(frame.numKPtsLeft()), Nright(frame.numKPtsRight()),
      ...
      mvMatches12(frame.mvMatches12), mnMatches(frame.mnMatches)
{
    for(int i=0;i<FRAME_GRID_COLS;i++)
        for(int j=0; j<FRAME_GRID_ROWS; j++){
            mGrid[i][j]=frame.mGrid[i][j];
            if(frame.numKPtsLeft() > 0){
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft,
        ...
        const float &thDepth, GeometricCamera* pCamera, EORB_SLAM::PairCalibPtr ppCalib, Frame* pPrevF, const IMU::Calib &ImuCalib)
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
...
#endif
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,0,0,true);
    thread threadRight(&Frame::ExtractORB,this,1,imRight,0,0,true);
    threadLeft.join();
    threadRight.join();
#ifdef SAVE_TIMES
    ...
    // assume undistorted image
    //vector<cv::KeyPoint> mvKeysRightUn;
    //ppCalib.first->undistKeyPoints(mvKeys, mvKeysUn);
    //ppCalib.second->undistKeyPoints(mvKeysRight, mvKeysRightUn);
    mvKeysUn=mvKeys;
    //mvKeysRight = mvKeysRightUn;
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    //UndistortKeyPoints();
    if (dynamic_cast<Pinhole*>(mpCamera)) {
        EORB_SLAM::MyCalibrator::undistKeyPointsPinhole(mvKeys, mvKeysUn, mK, mDistCoef, cv::Mat(), mK);
    }
    else {
        EORB_SLAM::MyCalibrator::undistKeyPointsFishEye(mvKeys, mvKeysUn, mK, mDistCoef, cv::Mat(), mK);
    }
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc,
        GeometricCamera* pCamera, const EORB_SLAM::MyCalibPtr& pCalib, cv::Mat &distCoef,
        const float &bf, const float &thDepth, Frame* pPrevF, const IMU::Calib &ImuCalib, const bool isMixed) :

     mpcpi(nullptr),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(nullptr)),
     mTimeStamp(timeStamp), mK(static_cast<Pinhole*>(pCamera)->toK()), mDistCoef(distCoef.clone()),
     mbf(bf), mThDepth(thDepth), mImuCalib(ImuCalib), mpImuPreintegrated(nullptr),mpPrevFrame(pPrevF),
     mpImuPreintegratedFrame(nullptr), mpReferenceKF(static_cast<KeyFrame*>(nullptr)), mbImuPreintegrated(false),
     mpCamera(pCamera), mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    // Frame ID
    mnId=nNextId++;
    ...
    if (!isMixed) {
        ...
        std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
        ExtractFeatures(0, imGray, 0, 1000);
#ifdef SAVE_TIMES
        std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
    ...
    if(mbInitialComputations)
    {
        ...
        // Do not change these to dynamic_cast!!!
    ...
    // If we have mixed features, don't assign features here!
    if (!isMixed) {
        AssignFeaturesToGrid();
    }
...
void Frame::ExtractFeatures(int flag, const cv::Mat &im, int x0, int x1, const bool extractDesc) {

    ExtractORB(flag, im, x0, x1, extractDesc);
}

void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1, const bool extractDesc)
{
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            ...
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                ...
                if(bCheckLevels)
                {
                    const int level = this->getKPtLevelMono(vCell[j]);
                    if(level<minLevel)
                        continue;
                    if(maxLevel>=0)
                        if(level>maxLevel)
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractor* extractorLeft,
        ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf,
        const float &thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2,
        cv::Mat& Tlr,Frame* pPrevF, const IMU::Calib &ImuCalib) :
        mpcpi(NULL), mpORBvocabulary(voc),mpORBextractorLeft(extractorLeft),mpORBextractorRight(extractorRight),
        mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
        mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF),mpImuPreintegratedFrame(NULL),
        mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbImuPreintegrated(false), mpCamera(pCamera),
        mpCamera2(pCamera2), mTlr(Tlr)
{
    ...
    thread threadLeft(&Frame::ExtractORB,this,0,imLeft,static_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[0],
            static_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[1],true);
    thread threadRight(&Frame::ExtractORB,this,1,imRight,static_cast<KannalaBrandt8*>(mpCamera2)->mvLappingArea[0],
            static_cast<KannalaBrandt8*>(mpCamera2)->mvLappingArea[1],true);
    threadLeft.join();
    threadRight.join();
...
inline cv::KeyPoint Frame::getUndistKPtMono(const int idx) const {
    assert(idx >= 0 && idx < N);
    return mvKeysUn[idx];
}

inline cv::KeyPoint Frame::getDistKPtMono(const int idx) const {
    assert(idx >= 0 && idx < N);
    return mvKeys[idx];
}

inline cv::KeyPoint Frame::getKPtRight(const int idx) const {
    assert(idx >= 0 && idx < mvKeysRight.size());
    return mvKeysRight[idx];
}

inline cv::Mat Frame::getORBDescriptor(const int idx) {
    assert(idx >= 0 && idx < N);
    return mDescriptors.row(idx);
}

inline cv::Mat Frame::getDescriptorMono(const int idx) {
    assert(idx >= 0 && idx < N);
    return mDescriptors.row(idx);
}

inline MapPoint* Frame::getMapPoint(const int idx) {
    assert(idx >= 0 && idx < N && mvpMapPoints.size() == N);
    return mvpMapPoints[idx];
}

inline MapPoint* Frame::getMapPoint(const int idx) const {
    assert(idx >= 0 && idx < N && mvpMapPoints.size() == N);
    return mvpMapPoints[idx];
}

inline void Frame::setMapPoint(const int idx, MapPoint *pMP) {
    assert(idx >= 0 && idx < N && mvpMapPoints.size() == N);
    //if (idx >= 0 && idx < N && mvpMapPoints.size() == N)
    this->mvpMapPoints[idx] = pMP;
    //else
        //cout << "Frame::setMapPoint: idx = " << idx << ", N = " << N << endl;
}

void Frame::setAllMapPointsMono(const std::vector<MapPoint *> &vpMPs) {
    if (!mvpMapPoints.empty())
        assert(mvpMapPoints.size() == vpMPs.size());
    this->mvpMapPoints = vpMPs;
}

inline bool Frame::getMPOutlier(const int idx) const {
    assert(idx >= 0 && idx < N && mvbOutlier.size() == N);
    return mvbOutlier[idx];
}

inline void Frame::setMPOutlier(const int idx, bool flag) {
    assert(idx >= 0 && idx < N && mvbOutlier.size() == N);
    this->mvbOutlier[idx] = flag;
}

void Frame::setAllMPOutliers(const std::vector<bool> &vbOutliers) {
    if (!mvbOutlier.empty())
        assert(vbOutliers.size() == mvbOutlier.size());
    this->mvbOutlier = vbOutliers;
}

/*inline float Frame::getORBScaleFactor(const int level) const {
    assert(level >= 0 && level < mnScaleLevels);
    return this->mvScaleFactors[level];
}

inline float Frame::getORBInvLevelSigma2(const int level) const {
    assert(level >= 0 && level < mnScaleLevels);
    return this->mvInvLevelSigma2[level];
}

inline int Frame::checkORBLevel(const int orbLevel) const {
...
inline int Frame::getKPtLevelMono(const int idx) const {

    return this->getUndistKPtMono(idx).octave;
}

inline float Frame::getKPtScaleFactor(const int idx) const {
...
inline float Frame::getKPtLevelSigma2(const int idx) const {
...
inline float Frame::getKPtInvLevelSigma2(const int idx) const {
...
int Frame::getMatches(const int idx) {

    if (mvMatches12.empty() || idx < 0 || idx >= mvMatches12.size())
        return -1;
    return mvMatches12[idx];
}
