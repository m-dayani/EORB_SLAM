...
//            }
    ...
    void System::EventSynchPreProcessing(FramePtr& pFrame) {

        if (mpEvTracker) {
            mpEvTracker->eventSynchPreProcessing(pFrame);
        }
    }

    void System::EventSynchPostProcessing() {

        if (mpEvTracker) {
            mpEvTracker->eventSynchPostProcessing();
        }
    }

    void System::SetInitEvFrameSynch(FramePtr& pFrame) {
    ...
    int System::TrackAndOptEvFrameSynch(FramePtr& pFrame) {
    ...
    void System::TrackEvFrameSynch(FramePtr& pFrame) {
    ...
    int System::EventLocalMappingSynch(ORB_SLAM3::KeyFrame* pEvKFcur) {

        if (mpEvTracker) {
            return mpEvTracker->evLocalMappingSynch(pEvKFcur);
        }
    }
    ...
    bool System::ResolveEventMapInit(FramePtr& pFrIni, KeyFrame* pKFini, FramePtr& pFrCurr, KeyFrame* pKFcur, const int nIteration) {

        if (mpEvTracker) {
            return mpEvTracker->resolveEventMapInit(pFrIni, pKFini, pFrCurr, pKFcur, nIteration);
        }
    }

    bool System::EvImReconst2ViewsSynch(const FramePtr& pFrame1, FramePtr& pFrame2,
            const std::vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
            std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated) {

        if (mpEvTracker) {
            return mpEvTracker->evImReconst2ViewsSynch(pFrame1, pFrame2, vMatches12,
    ...
    void System::ApplyScaleAndRotationEvSynch(const cv::Mat &R, const float scale, const bool bScaleVel, const cv::Mat& t) {

        if (mpEvTracker) {
            mpEvTracker->applyScaleAndRotationEvSynch(R, scale, bScaleVel, t);
        }
    }

    void System::UpdateEvFrameImuSynch(const float s, const IMU::Bias &b, KeyFrame *pOrbKFcur) {

        if (mpEvTracker) {
            mpEvTracker->updateEvFrameImuSynch(s, b, pOrbKFcur);
        }
    }
