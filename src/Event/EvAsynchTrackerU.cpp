    ...
    void EvAsynchTrackerU::saveAllFramePoses(const std::string &fileName, const double tsc, const int ts_prec) {
        ...
        if (isDisconnGraph()) {
            ...
            for (auto& pMap : mpEvAtlas->GetAllMaps()) {
                ...
                mFrameInfo.setIsLostRange(vpKFs[0]->mnId, vpKFs.back()->mnId, false, true);
...
//        mnLastRelocFrameId = mpCurrentFrame->mnId;
    ...
    void EvAsynchTrackerU::createCurrFrame(const PoseImagePtr &pImage, ulong fId, EvFramePtr &frame) {
        ...
        // we don't use the prior info in the current version
        ...
        VLOG_EVERY_N(50, 100) << "createCurrFrame: New frame created with Id: " << fId
                            << ", nPts: " << vAllKPts.size() << ", lastFrId: " << lastFrId << ", lastKfId: " << lastKfId << endl;
    ...
    void EvAsynchTrackerU::matchCurrentFrame(EvFramePtr &pCurFrame) {
        ...
        if (this->isMapInitialized()) {
            ...
            if (mbScaledTracking) {
                // if scaled tracking, level info must be considered in matches for init. map
                uint nMatches = pCurFrame->getNumMatches();
                vector<int> vMatches12 = pCurFrame->getMatches();
                ELK_Tracker::refineFirstOctaveLevel(mpIniFrame->getAllUndistKPtsMono(),
                                                    pCurFrame->getAllUndistKPtsMono(),vMatches12,nMatches);
    ...
    void EvAsynchTrackerU::removeOldFrames(ulong newRefId) {
        ...
        if (mmpWinFrames.empty() || newRefItr == mmpWinFrames.end() || newRefItr == mmpWinFrames.begin()) {
            VLOG(20) << "removeOldFrames: Conditions not right, abort at newRefId: " << newRefId << endl;
            return;
        }
    ...
    void EvAsynchTrackerU::trackLastFeatures(const PoseImagePtr &pImage) { // changes tracks
        ...
        double imTs = ((pImage) ? pImage->ts : -1);

        // return if feature tracks are not initialized
        if (!this->isTrackerInitialized()) {
            VLOG(2) << "trackLastFeatures: Tracker not initialized, ts: " << imTs << endl;
            return;
        }

        const bool tinyImage = pImage->mReconstStat == 0;
        ...
        const bool trackTinyFrames = mpEvParams->trackTinyFrames;
        ulong lastFrameId = mpLastFrame->mnId;
        unsigned nTracks = this->getTrackedPoints(lastFrameId, vAllKPts, vMapPoints, vFtTracks);
        // retrieve last tracked points
        vector<KeyPoint> lastTrackedKPts = mpLKTracker->getLastTrackedPts();

        if (nTracks <= 0 || vAllKPts.empty()) {
            LOG(WARNING) << "trackLastFeatures: Empty feature tracks, ts: " << imTs << endl;
            return;
        }

        VLOG_EVERY_N(50, 1000) << "trackLastFeatures: ScaledTracking? " << mbScaledTracking
                                            << ", tTiFr? " << trackTinyFrames << endl;

        if (!tinyImage || lastTrackedKPts.size() != vAllKPts.size()) {

            cv::Mat lastIm = mpLastFrame->imgLeft.clone();

            // init. LK Tracker
            mpLKTracker->setRefImage(lastIm, vAllKPts);
            VLOG_EVERY_N(50, 1) << "trackLastFeatures: New ref. set, tinyImage? " << tinyImage
                                 << ", lastPtsSz: " << lastTrackedKPts.size() << ", currPtsSz: " << vAllKPts.size() << endl;

            if (trackTinyFrames && vAllKPts.size() == lastTrackedKPts.size()) {
                VLOG_EVERY_N(50, 1) << "trackLastFeatures: Last tracked pts set\n";
                mpLKTracker->setLastTrackedPts(lastTrackedKPts);
            }

            if (mbScaledTracking && !tinyImage) {
                // extract reference descriptors
                VLOG_EVERY_N(50, 1) << "trackLastFeatures: Compute tracked descriptors\n";
                mpORBLevelDetector->ComputeTrackedKPtsDesc(lastIm, vAllKPts, allRefDescriptors);
            }
        ...
        // track
        mvLastMatches.clear();
        cv::Mat currIm = pImage->mImage.clone();
        mnLastTrackedKPts = mpLKTracker->trackAndMatchCurrImage(currIm, vAllKPts, mvLastMatches);

        VLOG_EVERY_N(50, 1) << "trackLastFeatures: Tracked last KPts, tinyFrame? " << tinyImage
                           << ", nLastTrackedPts: " << mnLastTrackedKPts << endl;

        // refresh point tracks only with MCFs
        if (!tinyImage) {
            if (mbScaledTracking) {
                // match scales using descriptors
                mpORBLevelDetector->AssignKPtLevelByBestDesc(allRefDescriptors, currIm, vAllKPts);
                VLOG_EVERY_N(50, 1) << "trackLastFeatures: Matched KPts levels by descriptors\n";
                //TODO: Keep descriptors for next iter
            }

            // process tracked points
            ulong currFrameIdx = (tinyImage) ? lastFrameId : mFrameIdx;
            this->updateTrackedPoints(currFrameIdx, vAllKPts, mvLastMatches, vFtTracks);

            // update track stats
            this->updateTrackStats();
            VLOG_EVERY_N(50, 1) << "trackLastFeatures: Updated tracked points, nFeatureTracks: "
...
//        }
    ...
    void EvAsynchTrackerU::detectAndFuseNewFeatures(const PoseImagePtr& pImage) {
        ...
        if (!this->isTrackerInitialized()) {
            ...
            VLOG(2) << "detectAndFuseNewFeatures: Not init. abort at currFrId: " << currFrameId << ", nNewKpts: "
                               << vNewKpts.size() << ", nFtTracks: " << mnFeatureTracks << endl;
            return;
        }
        ...
        VLOG(30) << "detectAndFuseNewFeatures: currFrId: " << currFrameId << ", nNewKpts: "
                << vNewKpts.size() << ", nFtTracks: " << mnFeatureTracks << endl;

        // remove old frames
        size_t lastSzFrames = mmpWinFrames.size();
        ulong minFrId = -1, maxFrId = 0;
        this->findTracksBounds(minFrId, maxFrId);
        this->removeOldFrames(minFrId);

        VLOG(30) << "detectAndFuseNewFeatures: Removed old frames, Size before rm: " << lastSzFrames
                           << ", Size Now: " << mmpWinFrames.size() << ", lowerId: " << minFrId << ", upperId: " << maxFrId << endl;
    }
...
//                if (save_test) {
//                    this->saveAllFramePoses("test_frame_poses.txt", 1.0, 9);
//                }
            }
            if (mStat == IDLE) {
