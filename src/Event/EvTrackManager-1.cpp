...
namespace EORB_SLAM
{
    ...
    EvTrackManager::EvTrackManager(const EvParamsPtr& pEvParams, const CamParamsPtr& camParams,
            ...
            sepThread(false), mStat(), mpL2AsynchTracker(nullptr), mpL2SynchTracker(nullptr),
            mpL2AsynchTrackerU(nullptr), mL1WinSize(pEvParams->l1ChunkSize),
...
//        }
    ...
    bool EvTrackManager::getRecommendedEvWinSize(unsigned long& winSize) {
        ...
        unsigned recommendedWinSize = l1ChunkSize * mL1NumLoop;
        unsigned long currBuffSize = 0;
        {
            std::unique_lock<mutex> lock(mMtxEvImQ);
            currBuffSize = mEvImBuffer.size();
        }
        winSize = min(static_cast<unsigned long>(recommendedWinSize), currBuffSize);
    ...
    bool EvTrackManager::reconstSynchEvMCI(PoseImagePtr& pPoseImage, const bool dispImage) {
        ...
        if (!winSizeRes) {
            DLOG_EVERY_N(WARNING, 100) << "EvTrackManager::reconstSynchEvMCI: "
                                          << imTs << ", Bad EvWinSize: " << evWinSize << endl;
            return winSizeRes;
        }
        ...
        DLOG_EVERY_N(WARNING, 1000) << "EvTrackManager::reconstSynchEvMCI: " << imTs
                    << ", Reconst. successfully: nEvs: " << vEvData.size() << ", tsEvEnd: "
                    << vEvData.back().ts << ", current SynchBuffSize: " << mEvImBuffer.size()
                    << ", L1 EvBuffSize: " << mEventBuffer.size() << endl;
    ...
    void EvTrackManager::setInitEvFrameSynch(ORB_SLAM3::Frame *pFrame) {
        ...
        if (reconstRes) {
            ...
            pImage->mTcw = cv::Mat::eye(4, 4, CV_32FC1);
            if (!pFrame->mTcw.empty()) {
                pImage->mTcw = pFrame->mTcw.clone();
            }
            mpL2SynchTracker->setInitEvFrameSynch(pImage, pFrame);
        }
    }

    void EvTrackManager::trackEvFrameSynch(ORB_SLAM3::Frame *pFrame) {
        ...
        if (reconstRes) {
            mpL2SynchTracker->trackEvFrameSynch(pImage->mImage, pFrame->mTimeStamp);
        }
    }
    ...
    int EvTrackManager::trackAndOptEvFrameSynch(ORB_SLAM3::Frame* pFrame) {
        ...
        int res = 0;
        if (reconstRes) {
            res = mpL2SynchTracker->trackAndOptEvFrameSynch(pImage->mImage, pFrame, currFrame);
        }
        // Optimize TODO: Do this inside Ev Tracker
        int nTrackedMPs = 0;
        if (res <= 0) {
            // If Event Map is not initialized, do a regular ORB pose optim.
            nTrackedMPs = ORB_SLAM3::Optimizer::PoseOptimization(pFrame);
        }
        else {
            // Else, do a full Ev-Im pose optimization
            nTrackedMPs = MyOptimizer::PoseOptimization(pFrame, currFrame);
            // Maybe refine or reinit. LK Tracker after successfull optimization
            // TODO: Correct this issue and activate this line
            //EvAsynchTracker::refineTrackedMPs(currFrame);
        }
        return nTrackedMPs;
    }
    ...
    void EvTrackManager::trackEvKeyFrameSynch(ORB_SLAM3::KeyFrame *pKFcur) {
        ...
        if (reconstRes) {
            mpL2SynchTracker->trackEvKeyFrameSynch(pImage->mImage, pKFcur);
        }
    }
    ...
    bool EvTrackManager::evImReconst2ViewsSynch(ORB_SLAM3::GeometricCamera* pCamera, const ORB_SLAM3::Frame* pFrame1,
            const ORB_SLAM3::Frame* pFrame2, const std::vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21,
            std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated) {

        PoseImagePtr pImage;
        getEmptyPoseImage(pImage, pFrame2->mTimeStamp);

        // Reconstruct MCI using L1 EvImBuilder
        bool res = this->reconstSynchEvMCI(pImage);

        // Dispatch MCI to main pipline of EvAsynchTracker
        int rStat = -1;
        if (res) {
            rStat = mpL2SynchTracker->evImReconst2ViewsSynch(pImage->mImage, pCamera, pFrame1, pFrame2, vMatches12,
                                                             R21, t21, vP3D, vbTriangulated);
        }

        // If cannot reconstruct using events, reconstruct with image points
        if (!res || rStat < 0) {
            return pCamera->ReconstructWithTwoViews(pFrame1->getAllUndistKPtsMono(), pFrame2->getAllUndistKPtsMono(),
                    vMatches12, R21, t21, vP3D, vbTriangulated);
        }
        else {
            return rStat != 0;
        }
