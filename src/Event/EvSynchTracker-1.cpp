...
namespace EORB_SLAM {
        ...
        while (!this->isStopped()) {
            ...
            if (isInputGood()) {
                ...
                if (pPoseImage->mReconstStat == 0) {
                    this->trackTinyFrameSynch(pPoseImage->mImage, pPoseImage->ts);
                }
    ...
    int EvSynchTracker::trackTinyFrameSynch(const cv::Mat &tinyIm, const double ts) {
    ...
    int EvSynchTracker::setInitEvFrameSynch(const PoseImagePtr& pImage, ORB_SLAM3::Frame* pFrame) {
    ...
    int EvSynchTracker::trackEvFrameSynch(const cv::Mat& mci, const double ts) {

        PoseImagePtr pImage = make_shared<PoseImage>(ts, mci, cv::Mat::eye(4, 4, CV_32FC1), "");
        ...
        if (!isTrackerInitializedSynch()) {
            ...
            return setInitEvFrameSynch(pImage, nullptr);
        }
        else {
            std::unique_lock<mutex> lock1(mMtxSynchTracker);
    ...
    int EvSynchTracker::trackAndOptEvFrameSynch(const cv::Mat& mci, ORB_SLAM3::Frame *pFrame, EvFrame& currEvFrame) {
        ...
        unsigned nMatches = 0;
        PoseImagePtr pImage = make_shared<PoseImage>(imTs, mci, cv::Mat(), "");
        {
            std::unique_lock<mutex> lock1(mMtxSynchTracker);
    ...
    int EvSynchTracker::trackEvKeyFrameSynch(const cv::Mat& mci, ORB_SLAM3::KeyFrame *pKFcur) {
        ...
        if (!isTrackerInitializedSynch() || mbSynchRefMatched == false) {
            ...
            this->setInitEvFrameSynch(pImage, nullptr);
            ...
            bool resMap = this->initMapSynch(*mpIniFrame, *mpLastFrame, mpReferenceKF, mpLastKF, medDepth);
            ...
            // Init. map using curr and ref. ORB KFs
            bool resMap = this->initMapSynch(*mpIniFrameTemp, *mpCurrFrame, mpReferenceKF, mpLastKF, medDepth);
    ...
    bool EvSynchTracker::initMapSynch(EvFrame& refFrame, EvFrame& curFrame,
        ...
        vector<MapPoint*> vpIniMPs = refKF->GetMapPointMatches();
    ...
    int EvSynchTracker::evImInitOptimizationSynch(const cv::Mat &currMCI, ORB_SLAM3::Map *pMap,
        ...
        if (!isTrackerInitializedSynch()) {
            ...
            setInitEvFrameSynch(pImage, nullptr);
            return stat;
        }
        ...
        if (!consistentTs) {
            trackEvKeyFrameSynch(currMCI, pKFcur);
            return stat;
        }
    ...
    int EvSynchTracker::evImReconst2ViewsSynch(const cv::Mat& currMCI, ORB_SLAM3::GeometricCamera* pCamera,
            const ORB_SLAM3::Frame* pFrame1, const ORB_SLAM3::Frame* pFrame2, const std::vector<int> &vMatches12,
            cv::Mat &R21, cv::Mat &t21, std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated) {

        int stat = -1;
        PoseImagePtr pImage = make_shared<PoseImage>(pFrame2->mTimeStamp, currMCI, cv::Mat::eye(4, 4, CV_32FC1), "");
        if (!isTrackerInitializedSynch()) {
