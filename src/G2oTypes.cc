...
void ImuCamPose::Update(const double *pu)
{
    ...
    // Update body pose
    twb += Rwb*ut;
    Rwb = Rwb*Converter::ExpSO3(ur);

    // Normalize rotation after 5 updates
    its++;
    if(its>=3)
    {
        Converter::NormalizeRotation(Rwb);
        its=0;
    }
...
void ImuCamPose::UpdateW(const double *pu)
{
    ...
    const Eigen::Matrix3d dR = Converter::ExpSO3(ur);
    DR = dR*DR;
    Rwb = DR*Rwb0;
    // Update body pose
    twb += ut;
    ...
    if(its>=5)
    {
        ...
        DR(2,0)=0.0;
        DR(2,1)=0.0;
        Converter::NormalizeRotation(DR);
        its=0;
    }
...
void EdgeInertial::computeError()
{
    ...
    const Eigen::Vector3d er = Converter::LogSO3(dR.transpose()*VP1->estimate().Rwb.transpose()*VP2->estimate().Rwb);
...
void EdgeInertial::linearizeOplus()
{
    ...
    const Eigen::Vector3d er = Converter::LogSO3(eR);
    ...
    _jacobianOplus[0].block<3,3>(3,0) = Converter::Skew(Rbw1*(VV2->estimate() - VV1->estimate() - g*dt)); // OK
    _jacobianOplus[0].block<3,3>(6,0) = Converter::Skew(Rbw1*(VP2->estimate().twb - VP1->estimate().twb
    ...
    _jacobianOplus[2].setZero();
    _jacobianOplus[2].block<3,3>(0,0) = -invJr*eR.transpose()*Converter::RightJacobianSO3(JRg*dbg)*JRg; // OK
...
void EdgeInertialGS::linearizeOplus()
{
    ...
    _jacobianOplus[0].block<3,3>(3,0) = Converter::Skew(Rbw1*(s*(VV2->estimate() - VV1->estimate()) - g*dt));
    _jacobianOplus[0].block<3,3>(6,0) = Converter::Skew(Rbw1*(s*(VP2->estimate().twb - VP1->estimate().twb
    ...
    _jacobianOplus[2].setZero();
    _jacobianOplus[2].block<3,3>(0,0) = -invJr*eR.transpose()*Converter::RightJacobianSO3(JRg*dbg)*JRg;
...
void EdgePriorPoseImu::computeError()
{
    ...
    const Eigen::Vector3d er = Converter::LogSO3(Rwb.transpose()*VP->estimate().Rwb);
...
void EdgePriorPoseImu::linearizeOplus()
{
    ...
    const Eigen::Vector3d er = Converter::LogSO3(Rwb.transpose()*VP->estimate().Rwb);
    _jacobianOplus[0].setZero();
    _jacobianOplus[0].block<3,3>(0,0) = Converter::InverseRightJacobianSO3(er);
