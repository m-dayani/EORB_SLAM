...
//BOOST_CLASS_EXPORT_GUID(KannalaBrandt8, "KannalaBrandt8")
    ...
    KannalaBrandt8::KannalaBrandt8(const std::vector<float>& _vParameters) : GeometricCamera(_vParameters),
        ...
        mD = (cv::Mat_<float>(4,1) << mvParameters[4], mvParameters[5], mvParameters[6], mvParameters[7]);
    }
    ...
    KannalaBrandt8::KannalaBrandt8(const std::vector<float>& _vParameters, TwoViewReconstruction* _tvr) :
            GeometricCamera(_vParameters, _tvr), precision(KB8_DEF_PRECISION), mvLappingArea(2, 0) {
    ...
    KannalaBrandt8::KannalaBrandt8(const std::vector<float>& _vParameters, const cv::Mat& _cvRmat, const cv::Mat& _cvPmat,
                                   TwoViewReconstruction* _tvr) :
            GeometricCamera(_vParameters, _tvr), precision(KB8_DEF_PRECISION), mvLappingArea(2, 0) {
    ...
    KannalaBrandt8::KannalaBrandt8(KannalaBrandt8* pKannala) : GeometricCamera(pKannala->mvParameters, pKannala->tvr),
        ...
        mK = pKannala->mK.clone();
        mD = pKannala->mD.clone();
    ...
    bool KannalaBrandt8::ReconstructWithTwoViews(const std::vector<cv::KeyPoint>& vKeys1, const std::vector<cv::KeyPoint>& vKeys2,
            const std::vector<int> &vMatches12, cv::Mat &R21, cv::Mat &t21, std::vector<cv::Point3f> &vP3D,
            std::vector<bool> &vbTriangulated){

        if(!tvr){
            tvr = new TwoViewReconstruction(mK);
        }

        //Correct FishEye distortion
        std::vector<cv::KeyPoint> vKeysUn1 = this->UndistortKeyPoints(vKeys1);
        std::vector<cv::KeyPoint> vKeysUn2 = this->UndistortKeyPoints(vKeys2);

        return tvr->Reconstruct(vKeysUn1, vKeysUn2, vMatches12,R21,t21,vP3D,vbTriangulated);
    }

    bool KannalaBrandt8::ReconstructWithTwoViews(const std::vector<cv::KeyPoint>& vKeys1,
        ...
        return tvr->Reconstruct(vKeysUn1,vKeysUn2,vMatches12,R21,t21,vP3D,
                vbTriangulated, vbTransInliers, reconstInfo);
    }
    ...
    std::vector<cv::KeyPoint> KannalaBrandt8::UndistortKeyPoints(const std::vector<cv::KeyPoint> &vKPts) {
        ...
        cv::fisheye::undistortPoints(vPts, vPts, mK, mD, mR, mP);
...
//                << mvParameters[0], 0.f, mvParameters[2], 0.f, mvParameters[1], mvParameters[3], 0.f, 0.f, 1.f);
    ...
    bool KannalaBrandt8::matchAndtriangulate(const cv::KeyPoint& kp1, const cv::KeyPoint& kp2, GeometricCamera* pOther,
        ...
        if((errX1*errX1+errY1*errY1)>KB8_DEF_CHISQ_COEF*sigmaLevel1){   //Reprojection error is high
            return false;
        }
        ...
        if((errX2*errX2+errY2*errY2)>KB8_DEF_CHISQ_COEF*sigmaLevel2){   //Reprojection error is high
            return false;
        }
