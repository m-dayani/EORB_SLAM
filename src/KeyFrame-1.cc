...
KeyFrame::KeyFrame():
        ...
        mnMaxY(0), /*mK(NULL),*/  mPrevKF(static_cast<KeyFrame*>(NULL)), mNextKF(static_cast<KeyFrame*>(NULL)),
        ...
        mbHasHessian(false), mnMergeCorrectedForKF(0), NLeft(0),NRight(0), mnNumberOfOpt(0),
        mpSynchEvKF(nullptr), mpSynchOrbKF(nullptr), mpOrigKF(nullptr)
{}

KeyFrame::KeyFrame(const KeyFrame& KF):
        mnFrameId(KF.mnFrameId),  mTimeStamp(KF.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
        ...
        mnBALocalForKF(KF.mnBALocalForKF), mnBAFixedForKF(KF.mnBAFixedForKF), mnBALocalForMerge(KF.mnBALocalForMerge),
        mnLoopQuery(KF.mnLoopQuery), mnLoopWords(KF.mnLoopWords), mnRelocQuery(KF.mnRelocQuery), mnRelocWords(KF.mnRelocWords),
        mnMergeQuery(KF.mnMergeQuery), mnMergeWords(KF.mnMergeWords), mnBAGlobalForKF(KF.mnBAGlobalForKF),
        fx(KF.fx), fy(KF.fy), cx(KF.cx), cy(KF.cy), invfx(KF.invfx), invfy(KF.invfy),
        mnPlaceRecognitionQuery(KF.mnPlaceRecognitionQuery), mnPlaceRecognitionWords(KF.mnPlaceRecognitionWords),
        mPlaceRecognitionScore(KF.mPlaceRecognitionScore), mbf(KF.mbf), mb(KF.mb), mThDepth(KF.mThDepth),
        N(KF.numAllKPts()), mvKeys(KF.mvKeys), mvKeysUn(KF.getAllUndistKPtsMono()),
        mvuRight(KF.mvuRight), mvDepth(KF.mvDepth), /*mDescriptors(NULL),*/
        /*mBowVec(NULL), mFeatVec(NULL),*/ mnScaleLevels(KF.getORBNLevels()), mfScaleFactor(KF.getORBScaleFactor()),
        mfLogScaleFactor(KF.getORBLogScaleFactor()), mvScaleFactors(KF.mvScaleFactors), mvLevelSigma2(KF.mvLevelSigma2),
        mvInvLevelSigma2(KF.mvInvLevelSigma2), mnMinX(KF.mnMinX), mnMinY(KF.mnMinY), mnMaxX(KF.mnMaxX), mnMaxY(KF.mnMaxY),
        /*mK(NULL),*/  mPrevKF(KF.mPrevKF), mNextKF(KF.mNextKF),
        mbFirstConnection(KF.mbFirstConnection), mpParent(KF.mpParent), mbNotErase(KF.mbNotErase),
        mbToBeErased(KF.mbToBeErased), mbBad(KF.mbBad), mHalfBaseline(KF.mHalfBaseline),
        mbCurrentPlaceRecognition(KF.mbCurrentPlaceRecognition),
        mbHasHessian(KF.mbHasHessian), mnMergeCorrectedForKF(KF.mnMergeCorrectedForKF), NLeft(KF.NLeft),
        NRight(KF.NRight), mnNumberOfOpt(KF.mnNumberOfOpt), mvpMapPoints(KF.mvpMapPoints), mpSynchOrbKF(KF.mpSynchOrbKF),
        mpSynchEvKF(KF.mpSynchEvKF), mpOrigKF(KF.mpOrigKF)
{}

KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
        bImu(pMap->isImuInitialized()), mnFrameId(F.mnId),  mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS),
        mnGridRows(FRAME_GRID_ROWS), mfGridElementWidthInv(F.mfGridElementWidthInv), mfGridElementHeightInv(F.mfGridElementHeightInv),
        mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnBALocalForMerge(0),
        mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0), mnRelocWords(0), mnBAGlobalForKF(0), mnPlaceRecognitionQuery(0),
        mnPlaceRecognitionWords(0), mPlaceRecognitionScore(0), fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy),
        mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.numAllKPts()), mvKeys(F.getAllORBDistKPtsMono()), mvKeysUn(F.getAllORBUndistKPtsMono()),
        mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.getAllORBDescMono().clone()),
        mBowVec(F.mBowVec), mFeatVec(F.mFeatVec), mnScaleLevels(F.getORBNLevels()), mfScaleFactor(F.getORBScaleFactor()),
        mfLogScaleFactor(F.getORBLogScaleFactor()), mvScaleFactors(F.getAllORBScaleFactors()), mvLevelSigma2(F.getAllORBLevelSigma2()),
        mvInvLevelSigma2(F.getAllORBInvLevelSigma2()), mnMinX(F.mnMinX), mnMinY(F.mnMinY), mnMaxX(F.mnMaxX),
        mnMaxY(F.mnMaxY), mK(F.mK), mPrevKF(NULL), mNextKF(NULL), mpImuPreintegrated(F.mpImuPreintegrated),
        mImuCalib(F.mImuCalib), mvpMapPoints(F.getAllMapPointsMono()), mpKeyFrameDB(pKFDB),
        mpORBvocabulary(F.mpORBvocabulary), mbFirstConnection(true), mpParent(NULL), mDistCoef(F.mDistCoef), mbNotErase(false),
        mnDataset(F.mnDataset), mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb/2), mpMap(pMap),
        mbCurrentPlaceRecognition(false), mNameFile(F.mNameFile), mbHasHessian(false), mnMergeCorrectedForKF(0),
        mpCamera(F.mpCamera), mpCamera2(F.mpCamera2), mvLeftToRightMatch(F.mvLeftToRightMatch),
        mvRightToLeftMatch(F.mvRightToLeftMatch),mTlr(F.mTlr.clone()), mvKeysRight(F.getAllORBKPtsRight()),
        NLeft(F.numKPtsLeft()), NRight(F.numKPtsRight()), mTrl(F.mTrl), mnNumberOfOpt(0), mpSynchOrbKF(nullptr),
        mpSynchEvKF(nullptr), mfScale(1.f), mpOrigKF(nullptr), mPriorTcw(F.getPosePrior())
{
    ...
    if(NLeft != -1)  mGridRight.resize(mnGridCols);
    for(int i=0; i<mnGridCols;i++)
    {
        mGrid[i].resize(mnGridRows);
        if(NLeft != -1) mGridRight[i].resize(mnGridRows);
...
KeyFrame::KeyFrame(EORB_SLAM::EvFrame &F, int kfId, Map *pMap, KeyFrameDatabase *pKFDB):
        bImu(pMap->isImuInitialized()), mnFrameId(F.mnId),  mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS),
        mnGridRows(FRAME_GRID_ROWS), mfGridElementWidthInv(F.mfGridElementWidthInv), mfGridElementHeightInv(F.mfGridElementHeightInv),
        mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnBALocalForMerge(0),
        mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0), mnRelocWords(0), mnBAGlobalForKF(0), mnPlaceRecognitionQuery(0),
        ...
        mpSynchEvKF(nullptr), mfScale(1.f), mpOrigKF(nullptr), mPriorTcw(F.getPosePrior())
{
    ...
    // Disable auto-indexing for event key frames
    mnId=kfId;//nNextId++;
...
vector<size_t> KeyFrame::GetFeaturesInArea(const float &x, const float &y, const float &r, const bool bRight) const
{
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        for(int iy = nMinCellY; iy<=nMaxCellY; iy++)
        {
            ...
            {
                const cv::KeyPoint &kpUn = (NLeft == -1) ? this->getUndistKPtMono(vCell[j])
                                                         : (!bRight) ? this->getDistKPtMono(vCell[j])
                                                                     : this->getKPtRight(vCell[j]);
...
inline cv::KeyPoint KeyFrame::getUndistKPtMono(const int idx) const {
    assert(idx >= 0 && idx < N);
    return this->mvKeysUn[idx];
}

inline cv::KeyPoint KeyFrame::getDistKPtMono(const int idx) const {
    assert(idx >= 0 && idx < N);
    return this->mvKeys[idx];
}

inline cv::KeyPoint KeyFrame::getKPtRight(const int idx) const {
    assert(idx >= 0 && idx < mvKeysRight.size());
    return this->mvKeysRight[idx];
}

inline cv::Mat KeyFrame::getORBDescriptor(const int idx) const {
    assert(idx >= 0 && idx < N);
    return this->mDescriptors.row(idx);
}

inline cv::Mat KeyFrame::getDescriptorMono(const int idx) {
    assert(idx >= 0 && idx < N);
    return this->mDescriptors.row(idx);
}

/*inline float KeyFrame::getORBScaleFactor(const int level) const {
    assert(level >= 0 && level < mnScaleLevels);
    return this->mvScaleFactors[level];
}

inline float KeyFrame::getORBLevelSigma2(const int level) const {
    assert(level >= 0 && level < mnScaleLevels);
    return this->mvLevelSigma2[level];
}

inline float KeyFrame::getORBInvLevelSigma2(const int level) const {
    assert(level >= 0 && level < mnScaleLevels);
    return this->mvInvLevelSigma2[level];
}

inline int KeyFrame::checkORBLevel(const int orbLevel) const {
...
inline int KeyFrame::getKPtLevelMono(const int idx) const {

    return this->getUndistKPtMono(idx).octave;
}

inline float KeyFrame::getKPtScaleFactor(const int idx) const {
    int level = this->getUndistKPtMono(idx).octave;
    //assert(level >= 0 && level < mnScaleLevels);
    return this->mvScaleFactors[this->checkORBLevel(level)];
}

inline float KeyFrame::getKPtLevelSigma2(const int idx) const {
    int level = this->getUndistKPtMono(idx).octave;
    //assert(level >= 0 && level < mnScaleLevels);
    return this->mvLevelSigma2[this->checkORBLevel(level)];
}

inline float KeyFrame::getKPtInvLevelSigma2(const int idx) const {
    int level = this->getUndistKPtMono(idx).octave;
    //assert(level >= 0 && level < mnScaleLevels);
...
void KeyFrame::PrintPointDistribution() {
    ...
    DLOG(INFO) << "Point distribution in KeyFrame: left-> " << left << " --- right-> " << right << endl;
}

string KeyFrame::printPointDistribution() {
    int left = 0, right = 0;
    int Nlim = (NLeft != -1) ? NLeft : N;
    for(int i = 0; i < N; i++){
        if(mvpMapPoints[i]){
    ...
    return "Point distribution in KeyFrame: left-> " + to_string(left) + " --- right-> " + to_string(right) + "\n";
}
